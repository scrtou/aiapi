...
REQUIRED = [
    ...
    ('AccountManager', 'setHttpTransport',
     r'\baccounts\s*->\s*setHttpTransport\s*\(\s*account::makeDrogonAccountHttpTransport\s*\(',
     r'\baccounts\s*->\s*init\s*\(', 'P7-W2·账号 HTTP adapter',
     '退化为未配置 transport：token refresh、registration 与上游删除全部失败'),
    ('AccountManager', 'setClock',
     r'\baccounts\s*->\s*setClock\s*\(\s*account::makeRealAccountClock\s*\(',
     r'\baccounts\s*->\s*init\s*\(', 'P7-W2·账号 clock adapter',
     '退化为未配置 clock：workflow poll/backoff 丢失真实时间语义'),
