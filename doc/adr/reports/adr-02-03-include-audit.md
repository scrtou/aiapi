...
真正的问题是 `target_include_directories` **被用成了全局** —— 一次性把 **30 个子目录**
全部加进搜索路径，效果与全局暴露等价。
...
| `src/CMakeLists.txt` | 唯一源清单 | `add_executable` + `target_include_directories`（:108，30 个目录）|
...
`src/CMakeLists.txt:108-137` 一次列出 **30 个目录**：
...
形式上仓库**已经满足**（确实只走 `target_include_directories`），但因为暴露了 30 个目录，
实际效果为零。
...
**那 200 处同目录 include 之所以能编译，完全依赖 §3 的 30 个目录暴露。**
...
> **阶段 1 前置检查项（新增，必做）**：验证头文件名唯一性。
> 若存在同名头文件（如两个 `Config.h`），机械改写会选错目标且**静默出错**。
> 成本约 10 分钟，**必须在动 CMake 之前做**。
...
| # | 原表述 | 修订 |
|---|---|---|
| 1 | 仅经 `target_include_directories` 暴露 | **补数量约束：每个 target 至多 1 个 include 目录**（`src/` 根）。当前 30 个 |
...
| 前置 | 无 | **验证头文件名全库唯一**（10 分钟，漏做则脚本静默出错）|
...
`src/test/CMakeLists.txt:58-93` 用 `PROJECT_SOURCES` **逐个列举**主程序 `.cpp`，
...
1. 测试只覆盖**被列举到的文件**，未列入的源文件测试侧根本不编译；
