...
删除 RFC 拆分前备份、两个旧 baseline 备份和四份已由当前 ADR/源码审计吸收的临时报告。
历史仍由 git 保存；`doc/adr/` 只保留当前决策、执行计划、机器基线和进行中的工作包产物。

## v3.4 文档分组与统一入口（2026-08-09）

将 RFC、设计、计划、审计、工作产物和历史按类型迁入独立目录；新增根 `README.md`，明确每个
目录/文件的职责、文档优先级和“migration-plan → workbook → work-products → 阶段门禁”的开发流程。
同步更新 CI、架构审计工具和全部相对链接。
...
| v2.5 | 2026-08-07 | **建立数字真值源，终结硬编码漂移**。新增 `doc/adr/audits/architecture-baseline.md`：以 `BL-*` 编号固化 R1/R2/R3 命中数、测试用例/断言数、测试链接覆盖数，附完整明细表、目录级缺口聚合与门禁契约；本文 16 处硬编码数字改为引用基线编号，冲突时以基线为准。**基线重跑并通过 `--selftest`**（`g++ -MM` 闭包模式，R2 由 23 → BL-R2，差值来自 `BackgroundTaskQueue.h` 新增单测后脱离零断言，非口径再次变动）；旧基线曾临时归档，v3.3 删除后由 git 历史保存。**校正 R3 表两处过期行号**（`transformRequestForToolBridge` 554/1660–2213 → 555/1660–2214；`accountManager.cpp` 1745–2103 → 1807–2165）。**P0-e1 结案**：`BackgroundTaskQueue::enqueue()` 的 `stopping_ \|\| shutdownCalled_` 门禁前置于懒启动分支，`shutdownCalled_` 为不可逆一次性标志，SIGTERM 后无线程池复活、无任务丢失、`shutdown()` 幂等，四阶段单测钉住。 |


## N8 取消检查点普查（2026-08-07）

**这是一次证伪。** N6 原始判断「设标志即可取消在途请求」建立在
「两个检查点覆盖阻塞区间」的未核实假设上。普查证明：两个检查点
（`GenerationService.cpp:221` / `:347`）夹住了 125 行耗时区间但不进入其中，
且 `APIinterface.h:28` 的 `generate(session_st&)` 签名不带令牌 ——
Provider 实现无法查询取消状态。**N6 不实施。**

附带发现三项（均为本次新增，此前无人记录）：
- 29 处 `sendRequest` 有 21 处无显式超时 → 新增 N10
- `IoLoopResponseStream::send()` 的断连返回值未接线 → 新增 N11
- `accountManager::backgroundSleep()` 是仓库唯一正确的可中断等待 → 定为 N7 模板

DB 边界经评估后**明确排除**，不留待办。工期 10.3 → 10.4 周。


## 评审第 8 条 · Result 错误模型收敛（2026-08-07）

裁决为采纳，但 ADR-05 v2.6 已答其大半，本次取证验证成立。新增三项此前未发现的事实：

- `chaynsapi::generate()` 是伪适配器：回读自己副作用写入的 `session.response.message`，
  错误码由 HTTP 状态码反推。RFC §0.3 推断「新旧路径并存」被证实，且实况更糟 ——
  不是并存，是新路径**寄生在**旧路径上
- `GenerationService.cpp:474-493` 与之构成往返双写，净效果为零但双点写入无法定责
- ADR-05 两个实测数字有误：catch 100 → 121，optional 59 → 35（结论方向不变，反而更强）

阶段 3 验收标准收紧（chaynsapi 24 处 `session.response` 清零），**不加工期**。
有利面：`AppError` 仅 4 个文件引用，尚未扩散，B1 成本处于最低点。


## ADR-02 / ADR-03 include 暴露方式实测（2026-08-07）

**三项排查全部结案。** ADR-02 原文要求的「阶段 1 建 target 前的 include 实测」已完成。

- ADR-02 手段 2 假设的敌人（全局 `include_directories()`）**一个都不存在**；
  真正的问题是 `target_include_directories` 被用成了全局 —— 一次暴露 28 个子目录
- 手段 1 只约束「经由什么机制暴露」，未约束「暴露多少」，**按原文当前仓库可判定合规** ——
  条款漏洞，已补数量约束（每 target 至多 1 个目录）
- ADR-03 真实规模是 **211 处 include 改写**（200 同目录 + 11 相对），此前从未量化；
  那 200 处正是靠 28 个目录才编译得过，收敛后立即全失败（+0.5 天）
- ADR-03 前置检查已执行：全库 `.h` **重名数 = 0**，机械改写前提成立；
  但须保留为守门项，改写当天重跑（阶段 0.5/1~3 会删改移动大量文件）
- 新增阶段 0.7 核对项：metrics ↔ dbManager 双向依赖是否为第四个环
- P12 补充：测试逐个列举 **32 个**主程序 `.cpp` 重编而非链接，主程序共约 80 个 ——
  **六成源文件测试侧根本不编译**
...
- **C6 落点改名**：`domain/model/AccountInfo.h` → **`domain/model/AccountData.h`**。
  ...
  **文档已对齐代码**（`migration-plan.md` :1165；详细源码事实已统一收口至
  `source-audit-2026-08.md`，历史报告可从 git 历史恢复）。
