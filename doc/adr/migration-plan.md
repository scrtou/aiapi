...
## ADR-02 / ADR-03 实测结论回写（2026-08-07）

报告：`doc/adr/reports/adr-02-03-include-audit.md`。**三项排查全部结案。**

1. **ADR-02 手段 2 无对象**：仓库无任何全局 `include_directories()`，降级为防退化约束，不计工作量。
2. **真问题是 `target_include_directories` 被用成全局**：`src/CMakeLists.txt:108-137` 暴露 **28 个子目录**。
   手段 1 原表述未约束「暴露多少」，**按原文当前仓库可判定合规** —— 已补数量约束：每 target 至多 1 个目录。
3. **ADR-02 新增手段 6**：CI 检查各 target include 列表一致。`src/` 28 个、`src/test/` 8 个，两份内容不同，已漂移。
4. **ADR-03 真实规模 211 处改写（+0.5 天）**：200 处同目录 include 正是靠上述 28 个目录才编译得过，
   收敛为单一根后立即全部失败。原文示例读起来像风格调整，实为全库机械改写。
5. **ADR-03 前置已执行**：全库 `.h` 重名数 = **0**，机械改写前提成立。
   但须保留为阶段 1 守门项 —— 阶段 0.5 删 Provider、阶段 1~3 移动文件后，**改写当天必须重跑**。
6. **阶段 0.7 新增核对项**：`metrics/ErrorStatsService.h` ↔ `dbManager/metrics/ErrorStatsDbManager.h`
   双向依赖是否属已登记三环之一；**若否则为第四个环**，阶段 0.7 范围需扩大。
7. **P12 补充**：`src/test/CMakeLists.txt:58-93` 逐个列举 **32 个**主程序 `.cpp` 重新编译，而非链接产物。
   主程序源文件约 80 个 —— **其余六成测试侧根本不编译**，新增文件不登记则静默漏测。
