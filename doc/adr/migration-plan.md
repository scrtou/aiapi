...
| **N6** | **`SessionExecutionGate::cancelAll()`**（停机 S0.5 广播取消） | 0.5 天 | 热修可行，建议随 N3 |
| **N7** | **8 处裸 `sleep_for` → `backgroundSleep`**（ADR-08 决策 3 收紧） | 0.5 天 | 热修，建议随 N3；含 3 个例外 |
...
**因此 N6 是本文档中实现成本最低的项之一**：约 10 行，无新抽象、无新依赖。
