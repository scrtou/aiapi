...
void ApiManager::init()
{
    ...
    LOG_INFO << "[接口管理器] 初始化完成";
    LOG_INFO << "[接口管理器] API名称映射大小：" << m_ApiNameApiMap.size();
}
...
std::shared_ptr<APIinterface> ApiManager::getApiByApiName(string apiName)
{
    ...
    {
        LOG_ERROR << "[接口管理器] API名称：" << apiName << " 未找到";
        return nullptr;
    }
