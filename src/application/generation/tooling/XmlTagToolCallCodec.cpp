...
// 哨兵标记（ 触发标记 ）
// 触发标记用于与 GenerationService 协同，示例：<Function_AB1c_Start/>。
...
/*
 * 实现说明：
 * 1. 本方法以 buffer_ 作为跨分片缓冲区，并通过 state_ 驱动有限状态机；当标签或内容尚未完整到达时，
 *    保留当前解析上下文并直接返回，待下一次 decodeIncremental() 追加数据后继续解析。
 * 2. Text 状态会先严格匹配本次会话配置的 sentinel_，只有命中触发标记后才把
 *    <function_calls>（同时兼容 antml 命名空间形式）识别为工具调用，标记之前及无法识别的内容按普通文本事件输出。
 * 3. 解析器兼容两种调用结构：<invoke name="..."><parameter ...>...</parameter></invoke>，以及
 *    <function_call><tool>...</tool><args_json>...</args_json></function_call>；各状态只消费已经确认完整的标签，
 *    并把工具名、调用编号、参数名和参数内容暂存在 currentContext_ 中。
 * 4. parameter 内容完成后立即生成 ToolCallArgsDelta；整个调用闭合时由 emitToolCallEnd() 统一校验、规范化并输出
 *    最终参数。args_json 支持 CDATA 原始 JSON 和 XML 转义文本，避免对 CDATA 再做反转义而破坏 JSON 结构。
 * 5. 对未知字符采用逐字符丢弃的方式重新同步；对疑似被分片截断的起始标签则暂留缓冲区，防止流式传输边界
 *    导致文本误发或工具调用漏判。attempt_completion 的参数允许包含 XML 示例，因此使用最后一个闭合标签作为边界，
 *    避免把示例中的标签误解析成新的工具调用。
 */
        ...
        switch (state_) {
            case XmlParserState::Text: {
                ...
                if (!sentinelMatched_) {
                    ...
                    // 继续循环，现在 sentinelMatched_ 为 ，会进入下方的 XML 解析逻辑
                    continue; 
                }
                // =======================================================
            ...
            case XmlParserState::InFunctionCalls: {
                ...
                if (buffer_.substr(0, 2) == "</") {
                    ...
                    // 注意：sentinelMatched_ 保持为 ，允许同一个块后续还有内容
                    // 如果需要每次块都重新匹配 ，这里应设为
                    break;
                }
...
static const char* stateName(XmlParserState s) {
  ...
  void XmlTagToolCallCodec::flush(std::vector<ToolCallEvent>& events) {
      if (!buffer_.empty()) {
          // 如果在 InArgsJson 状态，将 中剩余内容追加到参数值中再尝试恢复
          if (state_ == XmlParserState::InArgsJson) {
              ...
              buffer_.clear();
              LOG_INFO << "[XML工具调用编解码器] 恢复： InFunctionCall 状态，数据完整，直接";
              emitToolCallEnd(events);
              state_ = XmlParserState::Text;
              return;
          } else {
      ...
      if (state_ == XmlParserState::InArgsJson) {
          ...
          LOG_INFO << "[XML工具调用编解码器] ： InArgsJson 状态，尝试恢复不完整的 工具调用"
          ...
          if (tryRecoverIncompleteArgsJson(events)) {
              LOG_INFO << "[XML工具调用编解码器] ： 成功恢复不完整的 工具调用 (=" << currentContext_.toolName << ")";
              state_ = XmlParserState::Text;
              return;
          }
          // 恢复失败，走原有的 错误 路径
          LOG_WARN << "[XML工具调用编解码器] ： 恢复不完整 工具调用 失败 (=" << currentContext_.toolName << ")";
      }
  ...
  bool XmlTagToolCallCodec::tryRecoverIncompleteArgsJson(std::vector<ToolCallEvent>& events) {
      ...
      if (tryParseAndEmitArgs(payload, events)) {
          LOG_INFO << "[XML工具调用编解码器] 恢复成功： 直接解析 JSON";
          return true;
      }
      ...
      for (const auto& suffix : suffixes) {
          if (tryParseAndEmitArgs(payload + suffix, events)) {
              LOG_INFO << "[XML工具调用编解码器] 恢复成功： 补全后缀 '" << suffix << "'";
              return true;
          }
      }

      // 最后的兜底：如果 toolName 非空，尝试将整个 作为 raw_arguments 强制
      // 这样至少 工具调用 的 能传递给下游，让 applyStrict客户端Rules 有机会处理
      if (!currentContext_.toolName.empty()) {
          LOG_WARN << "[XML工具调用编解码器] 恢复： JSON 补全失败，强制 (=" << currentContext_.toolName << ")";
