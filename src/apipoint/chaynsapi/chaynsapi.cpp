...
void chaynsapi::init()
{
    ...
    // 从 config.json 的 custom_config.upstream_error_texts 加载上游错误文本列表
...
void chaynsapi::postChatMessage(session_st& session)
{
    ...
    // ========== 上游重试外层循环 ==========
    // 重试策略（三层）:
    //   内层: 同一线程上重复询问 SAME_THREAD_RETRIES 次
    ...
    while (totalAttempts < MAX_UPSTREAM_RETRIES && !upstreamSuccess) {
        ...
        // ---- 3. 处理图片上传 (仅首次) ----
        if (!imagesUploaded && !session.requestImages.empty()) {
        ...
        if (totalAttempts == 1 && !needSwitchAccount && session.is_continuation && !session.prev_provider_key.empty()) {
            std::lock_guard<std::mutex> lock(m_threadMapMutex);
            auto it = m_threadMap.find(session.prev_provider_key);
        ...
        // ========== 5. 同线程重试内层循环 ==========
        // 在同一线程上进行 SAME_THREAD_RETRIES 次尝试:
        //   第1次: 已经发送了消息，只需轮询结果
        ...
        for (int sameThreadAttempt = 1; sameThreadAttempt <= SAME_THREAD_RETRIES; ++sameThreadAttempt) {
            ...
            if (sameThreadAttempt > 1) {
                ...
                retryMessageBody["text"] = session.requestmessage;
                retryMessageBody["cursorPosition"] = (int)session.requestmessage.size();
    ...
    } // end of while (totalAttempts < MAX_UPSTREAM_RETRIES)
    ...
    if (upstreamSuccess) {
        ...
        session.responsemessage["message"] = final_response_message;
        session.responsemessage["statusCode"] = final_response_statusCode;
    } else {
        LOG_ERROR << "[chaynsAPI] 所有上游重试均失败 (总尝试次数: " << totalAttempts 
                 << "/" << MAX_UPSTREAM_RETRIES << ")";
        session.responsemessage["error"] = "Upstream failed after all retries";
        session.responsemessage["statusCode"] = 500;
    }
}
