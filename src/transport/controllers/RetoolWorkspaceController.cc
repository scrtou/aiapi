...
#include <transport/controllers/codecs/RetoolWorkspaceHttpJsonCodec.h>
    ...
    std::function<void(const HttpResponsePtr&)>&& callback)
{
    ...
    try
    {
        ...
        response["workspace"] = transport::workspacecodec::toJson(workspace, true);
    ...
    std::function<void(const HttpResponsePtr&)>&& callback)
{
    ...
    RetoolWorkspaceInfo info = transport::workspacecodec::fromJson(*jsonPtr);
    ...
    Json::Value response(Json::objectValue);
    response["status"] = "success";
    response["workspace"] = transport::workspacecodec::toJson(info, true);
    ctl::sendJson(callback, response, k200OK);
}
    ...
    std::function<void(const HttpResponsePtr&)>&& callback)
{
    ...
    for (const auto& record : records)
    {
        ...
        item["metadata"] = transport::workspacecodec::toJson(record, true);
